...
use clap::Parser;
use eframe::egui;
use rumble_egui::{Args, HotkeyManager, PersistentSettings, RumbleApp};
...
fn main() -> eframe::Result<()> {
    ...
    if let Some(rpc_command) = &args.rpc {
        ...
        if let Err(e) = rt.block_on(rumble_egui::rpc_client::run_rpc_command(socket_path, rpc_command)) {
