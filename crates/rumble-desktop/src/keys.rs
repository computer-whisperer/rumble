...
use rumble_client_traits::keys::{KeyInfo, KeySigning, KeySource};
...
impl KeySigning for NativeKeySigning {
    ...
    async fn get_signer(&self, public_key: &[u8; 32]) -> anyhow::Result<rumble_protocol::SigningCallback> {
        // Try local key first
...
/// Compute SHA256 fingerprint of a public key (matching rumble-egui format).
...
fn create_agent_signer_by_pubkey(public_key: [u8; 32]) -> rumble_protocol::SigningCallback {
